...
from tgbot.handlers.user import any_user, get_photo
...
bot = TeleBot(config.TOKEN, num_threads=5, parse_mode='HTML')

def register_handlers():
    bot.register_message_handler(admin_user, commands=['start'], admin=True, pass_bot=True)
    bot.register_message_handler(get_photo, content_types=['photo'], admin=False, pass_bot=True)
